			...
			if (userId) {
				...
				if (!itemExists) {
					...
					const updatedCart = userCarts.map((product) =>
						product.id === item.id ? newItem : product
					);
					await setDataToFirestoreRef('carts', userId, {
						carts: updatedCart,
						updatedAt: new Date().toDateString(),
		...
		try {
			if (userId) {
				...
				if (itemExists) {
					...
					toast.error(
						`${itemExists.qty}X items removed from cart successfully`
					);
					thunkAPI.dispatch(getAllCarts({ userId }));
				}
			} else {
...
const cartSlice = createSlice({
	...
	reducers: {
		...
		findCartTotal: (state, action) => {
			state.cartTotal = state.carts
				.reduce((curr, item) => curr + item.price * item.qty, 0)
				.toLocaleString('en-IN');
		},
		findFinalTotal: (state, action) => {
			state.finalTotal =
				state.cartTotal !== 0 && state.cartTotal < 500
					? parseInt(state.cartTotal) + 75
					: state.cartTotal;
		},
	},
});
